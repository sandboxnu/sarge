...
import TaskTemplateEditor from '@/lib/components/core/TaskTemplateEditor';
...
export default async function EditTaskTemplatePage({
    ...
    try {
        ...
        // Transform the Prisma result to match TaskTemplateDetail type
        const templateDetail: TaskTemplateDetail = {
            id: taskTemplate.id,
            title: taskTemplate.title,
            description: taskTemplate.description as TaskTemplateDetail['description'],
            tags: taskTemplate.tags,
            publicTestCases: (taskTemplate.publicTestCases as { input: string; output: string }[]) ?? [],
            privateTestCases: (taskTemplate.privateTestCases as { input: string; output: string }[]) ?? [],
            starterCodes: taskTemplate.starterCodes,
        };

        return <TaskTemplateEditor initialTemplate={templateDetail} />;
    } catch (error) {
        return (
            <div className="flex h-screen items-center justify-center">
                <div className="text-center">
                    <h1 className="text-2xl font-bold text-sarge-gray-900">
                        Task Template Not Found
                    </h1>
                    <p className="mt-2 text-sarge-gray-600">
