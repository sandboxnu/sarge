...
func initialModel() model {
	...
	m := model{
		...
		step: system,
		tasks: []task{
			{step: system, runningStr: "Checking system requirements", completedStr: "System requirements pass", run: fakeWork(2000 * time.Millisecond)},
			{step: permission, runningStr: "Checking root permissions", completedStr: "Verified root permissions", run: fakeWork(800 * time.Millisecond)},
			{step: dependencies, runningStr: "Installing dependencies", completedStr: "Dependencies successfully installed", run: fakeWork(1200 * time.Millisecond)},
...
// generateSSLCerts is the work that runs after the user submits a hostname.
// TODO: replace the sleep with the real cert-generation implementation.
func generateSSLCerts(hostname string) error {
	_ = hostname
	...
	case tea.KeyPressMsg:
		if m.step == hostname {
			...
			case "enter":
				...
				m.tasks[m.step].run = func() error { return generateSSLCerts(hostname) }
